...
## Maintenance chip 2026-09-25 ~10:18 PT — changelog-lag rule hardened + slice 2 complete
- Slice 2 (44 rows, two worker batches) — 39 changelog-top MATCHes, 5 DIFFERs all resolved as HOLDS:
  - Changelog-lag rule UPGRADED: the page's own text carries BOTH the changelog section AND the literal "Installer v" label, so corroboration is first-hand from a single fetch — grep the same page's text for "Installer v" and compare label-vs-label. When the label matches stored, the hold is double-sourced (not an inference). Mandatory step from this chip on: never call a changelog-lag hold on the changelog read alone. Cases corroborated: twintube v1.19.2, trinity-shaper v1.1.2, vss-2 v1.0.1, hitstrip v1.3.1, knocktonal v1.2.1.
  - No-changelog-page class (exemplar: hears-perfection): page fetches fine, title matches, NO Changelog section at all. This is a version signal (nothing shipped since stored 1.0.0), distinct from a fetch failure/block. Hold at stored, re-probe next rotation, never log as a block.
  - Swivel Audio title note: "Swivel Audio HitStrip"/"Swivel Audio Knocktonal" is a vendor-prefix title, NOT the bx_boom free-repurpose class — same product, valid signal. Repurpose guard triggers only when the title identifies a DIFFERENT product.
  - elysia-mpressor (cenozoix identity fix): reads true at 1.17.0 — seed-notes correction holding.
- Batch 1 (20 MATCH, 2 DIFFER): ds-tantra-2 changelog uses DS Audio's own format ("2023.08.23: DS Tantra 2 v2.0.2") — parse the version token, not the date. Bax-eq master/mix share /products/bax-eq — fetch once, report both.
- Remaining stale PA block: 164 rows (verified < 2026-09-25). Next chip: slice 3, same recipe.
